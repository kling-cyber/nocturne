...
io.on("connection",socket=>{
  ...
  socket.on("launchCase",async()=>{
    ...
    if(!room)return fail(socket,"You are not in a case room.");
    if(room.mode==="SINGLE_PLAYER")return fail(socket,"Single-player cases start automatically.");
    if(room.hostId!==socket.id)return fail(socket,"Only the case host can start the case.");
    if(room.players.size<2)return fail(socket,"At least 2 human players are required for Multiplayer. Use Single Player for solo play.");
    try{
      await room.start();
      if(!installRoles(room,"case-start"))throw new Error("Role controller was not installed");
      room.sim.emit();
      console.log("[NOCTURNE] multiplayer role system ready",room.code);
    }catch(error){
      console.error("[NOCTURNE] Multiplayer start error:",error);
      fail(socket,"Unable to start the multiplayer case.");
    }
  });

  socket.on("createSinglePlayer",async payload=>{
    const name=clean(payload?.name,24);if(!validName(name))return fail(socket,"Choose a name between 1 and 24 characters.");
    const investigatorRole=clean(payload?.investigatorRole,60);const difficulty=clean(payload?.difficulty,30).toUpperCase();
    const code=roomCode();const room=new GameRoom(code,io);room.mode="SINGLE_PLAYER";room.singlePlayer=true;rooms.set(code,room);
    try{if(typeof room.startSinglePlayer!=="function"){rooms.delete(code);return fail(socket,"Single-player mode is not available in this server build yet.");}await room.startSinglePlayer(socket,{name,requestedInvestigatorRole:investigatorRole,difficulty});}
    catch(error){console.error("[NOCTURNE] Single-player start error:",error);rooms.delete(code);fail(socket,"Unable to start the single-player case.");}
  });
  ...
  socket.on("roleAction",async payload=>{
    ...
    const installed=installRoles(room,"role-action");
    if(!installed){
      console.error("[NOCTURNE] role runtime unavailable",room.code,typeof room.sim.roleAction,room.sim.phase);
      return socket.emit("roleActionResult",{ok:false,action,message:"The multiplayer role controller is not available in this server process."});
    }
    ...
    try{
      ...
      if(!result||typeof result!=="object"){
        ...
        return socket.emit("roleActionResult",{ok:false,action,message:"The role ability did not return a result from the case engine."});
      }
      if(result.ok){
  ...
  socket.on("requestVisual",async payload=>{
    const room=findRoom(socket);if(!room||!room.sim)return;const actor=room.sim.get(socket.id);if(!actor||!actor.alive||room.sim.caseClosed)return;
    const result=await visualBatch.directRequest(room.sim,socket.id,{type:payload?.type,cameraId:clean(payload?.cameraId,30)});if(result?.error)return fail(socket,result.error);
    const asset=result.asset;const title=asset.cameraId+' // '+asset.kind.toUpperCase()+' // '+asset.clock;
    const description=asset.kind==='cctv'?`Directly generated CCTV evidence from ${asset.area} at ${asset.clock}. The frame captures ordinary movement, occupancy, spatial relationships and environmental detail from a fixed security-camera viewpoint. Treat visible people as observations of presence and movement only, not proof of identity or intent. Compare entrances, exits, timing, occlusion and repeated appearances against testimony and access records. Limitations include camera blind spots, compression, lighting, reflections and partial views.`:`Directly generated investigative scene photograph from ${asset.area} at ${asset.clock}. The photograph records physical layout, environmental condition and a subtle observation selected for cross-reference. Separate what is visibly present from what you infer about it, then compare the scene with timeline, movement and testimony evidence. Limitations include angle, lighting, prior disturbance and the possibility of innocent explanations.`;
    room.sim.add({type:'visual',title,description,reliability:asset.kind==='cctv'?75:65,source:asset.cameraId,visualAssetId:asset.id,image:asset.image,area:asset.area,captureClock:asset.clock});
    room.sim.event('VISUAL',actor.name+' generated '+asset.kind.toUpperCase()+' evidence from '+asset.cameraId+'.');
    socket.emit('visualReady',{title,description,reliability:asset.kind==='cctv'?75:65,image:asset.image});room.sim.emit();
  });

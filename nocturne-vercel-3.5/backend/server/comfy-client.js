...
// Exact API graph exported from the working Realistic Vision workflow supplied
// for NOCTURNE. The node IDs intentionally mirror the exported ComfyUI JSON.
function baseWorkflow({ prompt, negativePrompt, seed }) {
  return {
    ...
    "75": {
      ...
      inputs: { text: clean(negativePrompt), clip: ["73", 1] }
    },
    "76": {
      class_type: "CLIPTextEncode",
      inputs: { text: clean(prompt), clip: ["73", 1] }
    },
    "77": {
      class_type: "KSampler",
      inputs: {
...
async function uploadReference(buffer, filename) {
  ...
  if (!response.ok) {
    throw new Error(`ComfyUI /upload/image HTTP ${response.status}: ${clean(text, 1200)}`);
  }
  if (!data?.name) {
...
async function request({ prompt, negativePrompt, caseSeed, cameraId, type, capture = 0, initImage = null }) {
  ...
  console.log(
    `[NOCTURNE] ComfyUI request: ${type} ${cameraId} ${WIDTH}x${HEIGHT} steps=${STEPS} cfg=${CFG} sampler=${SAMPLER} scheduler=${SCHEDULER} continuity=${continuity} timeout=${GENERATION_TIMEOUT_MS}ms`
  );
  ...
  if (!response.ok) {
    throw new Error(`ComfyUI /prompt HTTP ${response.status}: ${clean(text, 1200)}`);
  }
  if (data?.error) {
    throw new Error(`ComfyUI rejected workflow: ${clean(JSON.stringify(data.error), 1500)}`);
  }
  ...
  while (Date.now() < deadline) {
    ...
    if (entry.status?.status_str === "error") {
      const messages = Array.isArray(entry.status?.messages)
        ? entry.status.messages.map(x => JSON.stringify(x)).join(" ")
    ...
    for (const node of Object.values(entry.outputs || {})) {
      ...
      if (!imageResponse.ok) {
        throw new Error(`ComfyUI /view HTTP ${imageResponse.status}.`);
      }

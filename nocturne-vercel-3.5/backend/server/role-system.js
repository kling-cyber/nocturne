...
// Role actions deliberately write to ONE investigative surface: concrete findings
// become Evidence; meaningful public actions become Case Feed events. They are
// not duplicated into both places.
...
function output(room,pid,result){const out={ok:true,roleAction:true,title:clean(result?.title||'Role ability resolved.',140),description:clean(result?.description||'The role ability completed successfully.',900),action:clean(result?.action||'',120)};room.io.to(pid).emit('roleActionOutput',out);return out;}
 ...
 sim.roleAction=function(pid,raw){
  ...
  if(p.role===CORE.killer){
   ...
   if(lower.startsWith('kill ')){
    ...
    sim.crime();sim.emit();
    return output(room,pid,{action:text,title:`Target selected: ${target.name}`,description:`The target selection resolved the critical event. The public case record now reflects the resulting crime.`});
   }
   if(lower==='conceal scene'){
    if(!sim.truth.crimeCommitted)return notice(room,pid,'There is no crime scene to conceal yet.');
    sim.add({type:'environmental',title:'Possible scene interference',description:'The crime scene shows signs of limited post-event disturbance. Investigators should corroborate the scene carefully.',reliability:52,source:'scene'});
    sim.tick(1);sim.emit();
    return output(room,pid,{action:text,title:'Scene concealment attempted',description:'You disturbed part of the crime scene. The resulting environmental finding is now available to investigators in Evidence.'});
   }
  ...
  if(p.role===CORE.detective){
   if(lower==='analyze case'){
    ...
    sim.add({type:'analysis',title:'Detective case analysis',description:`The Detective cross-referenced ${recent.length} recent evidence records. Average reported reliability is ${avg}%. The strongest leads should be compared for timing, location and source consistency. This is an analytical lead, not automatic proof.`,reliability:Math.min(95,avg+10),source:p.name});
    sim.emit();return output(room,pid,{action:text,title:'Case analysis complete',description:`${p.name} cross-referenced ${recent.length} recent evidence records. The resulting analysis is in Evidence. No separate Case Feed entry was created because this was an analytical finding, not a public event.`});
   }
   if(lower.startsWith('interrogate ')){
    ...
    return output(room,pid,{action:text,title:`Interrogation opened: ${target.name}`,description:`A formal question was sent to ${target.name}. The response will enter Evidence as testimony, where it can be compared against the timeline and independent records.`});
   }
   if(lower.startsWith('mark suspect ')){
    ...
    sim.add({type:'behavioral',title:`Person of interest: ${target.name}`,description:`The Detective marked ${target.name} for focused review. Suspicion increased by 12 points. This is an investigative designation, not a verdict.`,reliability:70,source:p.name});
    sim.emit();return output(room,pid,{action:text,title:`${target.name} marked for review`,description:`${target.name} was marked as a person of interest. The designation is stored in Evidence so the team can compare it with later findings.`});
   }
  ...
  if(p.role===CORE.investigator){
   if(lower==='forensics'){
    sim.add({type:'trace',title:`Forensic search: ${p.location}`,description:`${p.name} performed a focused field-forensics search in ${p.location}, checking traces, objects and environmental inconsistencies. The finding identifies leads requiring corroboration; it does not identify a culprit by itself.`,reliability:76,source:p.name});
    sim.emit();return output(room,pid,{action:text,title:`Forensics complete: ${p.location}`,description:`The forensic search produced a trace finding in Evidence. Because this is a concrete investigative finding, it is not duplicated in Case Feed.`});
   }
   if(lower.startsWith('track ')){
    ...
    sim.add({type:'observation',title:`Movement trace: ${target.name}`,description:`${p.name} followed the movement trail of ${target.name} from ${old} toward ${target.location}. This is a movement lead and should be checked against other observations and timestamps.`,reliability:72,source:p.name});
    sim.emit();return output(room,pid,{action:text,title:`Tracking complete: ${target.name}`,description:`The movement trace was recorded in Evidence. The Case Feed is not duplicated because the useful result is the trace itself.`});
   }
   if(lower==='recon'){
    const nearby=sim.people.filter(x=>x.alive&&x.location===p.location&&x.id!==p.id).map(x=>x.name);
    sim.add({type:'observation',title:`Recon: ${p.location}`,description:`Field reconnaissance found ${nearby.length?nearby.join(', '):'no other visible people'} in ${p.location} at the time of inspection. Presence or absence is a lead and can change as people move.`,reliability:68,source:p.name});
    sim.emit();return output(room,pid,{action:text,title:`Recon complete: ${p.location}`,description:`The reconnaissance observation is now in Evidence. It is not duplicated in Case Feed because recon produces an observation rather than a public event.`});
   }

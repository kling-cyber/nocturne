...
function context(sim,p){
  ...
  let memories=[];try{if(typeof sim.recall==='function')memories=sim.recall(p,'').slice(-6).map(m=>({type:m.type,text:m.text,confidence:m.confidence,importance:m.importance}));}catch(error){console.warn('[NOCTURNE] Role memory context unavailable:',error?.message||error);}
...
function extractOutputText(data){
  ...
  const parts=[];for(const item of data?.output||[])for(const c of item?.content||[])if(typeof c?.text==='string')parts.push(c.text);return parts.join('');
}
async function aiResolve(sim,p,action,allowedEffect){
  const key=String(process.env.OPENAI_API_KEY||'').trim();if(!key){console.warn('[NOCTURNE] Role AI disabled: OPENAI_API_KEY is missing.');return null;}
  const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),AI_TIMEOUT);
  try{
    ...
    console.log('[NOCTURNE] Role AI start',p.name,action,MODEL);
    ...
    const data=await response.json();const text=extractOutputText(data);if(!text){console.error('[NOCTURNE] Role AI returned no output text');return null;}
    const x=JSON.parse(text);console.log('[NOCTURNE] Role AI complete',p.name,action,x.outcome);
...
function install(room){
  ...
  sim.__nocturneRoleLocks=new Set();
  sim.roleAction=async function(pid,raw){
    ...
    if(sim.__nocturneRoleLocks.has(pid))return notice(room,pid,'Your previous role ability is still being resolved. Please wait for its result.');
    ...
    }finally{sim.__nocturneRoleLocks.delete(pid);}
  };
  ...
  sim.event('ROLES','The investigation team has been assigned. Human role assignment is randomized after the authoritative killer is selected. Missing human roles are represented by autonomous NPC specialists.');sim.emit();
}

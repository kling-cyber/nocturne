...
const AI_TIMEOUT=Number(process.env.NOCTURNE_ROLE_AI_TIMEOUT_MS||7500);
...
function context(sim,p){
  ...
  const memories=typeof sim.recall==='function'?sim.recall(p,'').slice(-6).map(m=>({type:m.type,text:m.text,confidence:m.confidence,importance:m.importance})):[];
...
async function aiResolve(sim,p,action,allowedEffect){
  ...
  try{
    ...
    if(!response.ok){console.error('[NOCTURNE] Role AI HTTP',response.status,await response.text().then(x=>x.slice(0,300)).catch(()=>''));return null;}
  ...
  sim.roleAction=async function(pid,raw){
    ...
    if(p.role===CORE.killer){
      ...
      if(lower==='conceal scene'){
        if(!sim.truth.crimeCommitted)return notice(room,pid,'There is no crime scene to conceal yet.');sim.tick(1);
